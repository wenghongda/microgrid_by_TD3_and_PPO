...
class TD3:
    def __init__(self,env,n_games: int = 5, training: bool = True,
                 alpha=0.0005, beta=0.0005, gamma=0.9, tau=0.004,
                 batch_size: int = 96, noise: str = 'normal'):
    ...
    def train(self):
        ...
        for i in range(self.n_games):
            ...
            reward_total = (reward_total - reward_total.mean())/(reward_total.std()+1e-7)
            idx_for_training = random.sample(range(len(memory_total)//batch_size),1)[0]
            memory_for_training = memory_total[idx_for_training:idx_for_training+batch_size]
